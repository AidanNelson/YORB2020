...
async function runSocketServer() {
  ...
  io.on('connection', (socket) => {
    ...
    //     rotation: [0, 0, 0, 1] // stored as XYZW values of Quaternion
    //   }
    //   // clients[socket.id].uuid = uuid;
    //   // console.log('updating client with id ' + socket.id + " to have uuid " + data);
    //   // io.sockets.emit('newUserConnected', io.engine.clientsCount, uuid, Object.keys(clients));
    // })

    socket.on('move', (data) => {
      // if (clients[data.id]) {
      //   clients[data.id].position = data[0];
      //   clients[data.id].rotation = data[1];
      // }
      if (roomState.peers[data.id]) {
        roomState.peers[data.id].gameStats.position = data[0];
        roomState.peers[data.id].gameStats.rotation = data[1];
      }
