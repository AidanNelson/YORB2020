...
export class SpringShow {
  constructor(scene, camera, controls) {
    ...
    this.linkHighlightedMaterial = new THREE.MeshLambertMaterial({ color: 0xffff1a });
  ...
  createHyperlinkedMesh(x, y, z, _project) {
    ...
    if (status_code == "1") {
      ...
      status = "Live now!";
      var statusTextMesh = createSimpleText(
        status,
        statusColor,
        fontSize,
        this.font
  ...
  highlightLink(link) {
    ...
    link.children[0].material = this.linkHighlightedMaterial;
